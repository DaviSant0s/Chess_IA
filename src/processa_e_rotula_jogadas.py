...
def rotula_jogadas_stockfish():
    ...
    for i, item in enumerate(jogadas):
        ...
        info_antes = engine.analyse(board, chess.engine.Limit(depth=8))
        ...
        info_depois = engine.analyse(board, chess.engine.Limit(depth=8))
        ...
        if i % 50 == 0:
            ...
            print(f"[{tempo_passado:.2f}s] Avaliado {i+1}/{len(jogadas)} jogadas...")
